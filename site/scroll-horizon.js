...
(function () {
  ...
  var REST = 0.20;    // resting crest height = 20% from top — crest rests ~4/5 up the screen (do not touch)
  var START = 0.75;   // scroll-top crest: a thin lifted ocean band visible from the top (not pinned to the very bottom)
  ...
  function compute() {
    ...
    if (MODE === 'scroll') {
      // WHOLE-PAGE proportional: the crest is a progress indicator for the ENTIRE page.
      //   p = scrollY / (scrollHeight - vh)
      // so the water-line rises steadily from START·vh (top) to REST·vh (bottom) across
      // the full document height — short pages included, since p still spans 0→1 over
      // their own scroll. A smoothstep ease holds the crest gracefully at both ends.
      var p = max > 0 ? Math.min(1, Math.max(0, sy / max)) : 0;
      var pe = p * p * (3 - 2 * p);                 // smoothstep — eased, no sudden jumps
      tgtY = (START - pe * (START - REST)) * vh;
    } else {
