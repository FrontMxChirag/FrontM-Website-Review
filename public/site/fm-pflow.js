...
(function () {
  ...
  /* ---- $52K+ odometer: counts up through 4-digit numbers to 9,999, flips to
     K-notation (10K → 52K), and drops the + sign near the end. Plays once on
     reveal; rewinds if the viewer scrolls back above the beat. ---- */
  var gxEl = stat ? stat.querySelector('.gx') : null;
  var GX_TARGET = 52000, GX_DUR = 1500, gxRaf = 0, gxPlayed = false;
  function gxFmt(v){
    v = Math.max(0, v);
    var plus = v >= GX_TARGET * 0.985 ? '+' : '';
    if (v < 10000) return '$' + Math.round(v).toLocaleString('en-US') + plus;
    return '$' + Math.round(v / 1000) + 'K' + plus;
  }
  function gxRun(){
    if (!gxEl) return;
    var t0 = performance.now();
    cancelAnimationFrame(gxRaf);
    (function step(now){
      var t = Math.min(1, (now - t0) / GX_DUR);
      var e = 1 - Math.pow(1 - t, 3);   // ease-out
      gxEl.textContent = gxFmt(e * GX_TARGET);
      if (t < 1) gxRaf = requestAnimationFrame(step);
      else gxEl.textContent = '$52K+';
    })(t0);
  }
  ...
  function applyPhases(){
    ...
    if (gxEl && !reduced){
      if (stIn > 0.5 && !gxPlayed){ gxPlayed = true; gxRun(); }
      else if (stIn < 0.05 && gxPlayed){ gxPlayed = false; cancelAnimationFrame(gxRaf); gxEl.textContent = '$52K+'; }
    }
  ...
  /* ---- finale glow tracks the cursor: the radial light behind the giant title
     drifts toward the pointer (eased), so the title/constellation reads as lit from
     wherever you are on screen. Pure CSS-var write — no layout, no clipping (the head
     is overflow:visible now). Fine pointers only; reduced-motion leaves it centred. ---- */
  if (wheelTitle && !reduced && matchMedia('(pointer: fine)').matches){
    var glTX = 0, glTY = 0, glX = 0, glY = 0, glRaf = 0;
    var glStep = function (){
      glRaf = 0;
      glX += (glTX - glX) * 0.08; glY += (glTY - glY) * 0.08;
      wheelTitle.style.setProperty('--glx', glX.toFixed(2) + '%');
      wheelTitle.style.setProperty('--gly', glY.toFixed(2) + '%');
      if (Math.abs(glTX - glX) + Math.abs(glTY - glY) > 0.05) glRaf = requestAnimationFrame(glStep);
    };
    window.addEventListener('pointermove', function (e){
      var nx = (e.clientX / window.innerWidth) * 2 - 1;
      var ny = (e.clientY / window.innerHeight) * 2 - 1;
      glTX = Math.max(-1, Math.min(1, nx)) * 16;   // ±16% horizontal travel
      glTY = Math.max(-1, Math.min(1, ny)) * 18;   // ±18% vertical travel
      if (!glRaf) glRaf = requestAnimationFrame(glStep);
    }, { passive: true });
  }
